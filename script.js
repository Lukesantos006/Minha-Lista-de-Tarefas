...
let modoEdicao = false; // Controla se estamos editando uma tarefa existente
let indiceEdicao = null; // Guarda o índice da tarefa que está sendo editada
...
function adicionarTarefa() {
  ...
  let duplicada = false;
  const tarefas = obterTarefas(); //obtém tarefas já existentes

  if (modoEdicao && indiceEdicao !== null) { // Se estamos em modo de edição
    tarefas[indiceEdicao].texto = texto; // Atualiza o texto
    tarefas[indiceEdicao].data = data; // Atualiza a data
    tarefas[indiceEdicao].prioridade = prioridade; // Atualiza a prioridade
    guardarTarefas(tarefas); // Salva no localStorage

    document.getElementById("listaTarefas").innerHTML = ""; // Limpa a lista
    tarefas.forEach(t => mostrarTarefaNoEcrã(t)); // Re-renderiza todas as tarefas
    atualizarContadorPendentes(); // Atualiza contador
    filtrarTarefas(); // Reaplica filtro
    
    document.getElementById("btnAdicionar").textContent = "Adicionar"; // Restaura botão
    inputTexto.value = ""; // Limpa campos
    inputData.value = "";
    inputPrioridade.value = "baixa";
    modoEdicao = false;
    return; // Sai da função sem adicionar nova tarefa
  }

  for (i=0; i < tarefas.length; i++) {
    if (tarefas[i].texto === inputTexto.value) {
      duplicada=true;
    }  
  }

  if (texto !== "" && duplicada == false) { // Verifica se o texto não está vazio
...
if (tarefa.data && tarefa.data < hoje && !tarefa.concluida) {
  ...
  botaoConcluir.classList.add("finished");
...
function editarTarefas(botao) {
  const li = botao.parentElement.parentElement; //Ir buscar o li da tarefa correspondente ao botão
  const span = li.querySelector("span");
  const atualTarefa = document.getElementById("novaTarefa");
  const textoAtual = span.textContent;
  const inputData = document.getElementById("dataTarefa");
  const inputPrioridade = document.getElementById("prioridadeTarefa");

  const regex = /^(.*) \(até (.*)\)$/;
  const matches = textoAtual.match(regex);
  ...
  if (matches) {
    nomeAtual = matches[1];
    dataAtual = matches[2]; 
    atualTarefa.value = nomeAtual;
    inputData.value = dataAtual;
    inputPrioridade.value = prioridadeAtual;
  }

  else { 
       atualTarefa.value = textoAtual;
       inputPrioridade.value = prioridadeAtual;
    }

    modoEdicao = true;
    indiceEdicao = Array.from(document.getElementById("listaTarefas").children).indexOf(li);
    document.getElementById("btnAdicionar").textContent = "Salvar";
